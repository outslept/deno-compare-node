...
export const inspect = (value, indentLevel = "", cache = new Map()) => {
  if (value === undefined) {
    return "undefined";
  }
  ...
  if (["boolean", "number", "symbol"].includes(valueType)) {
    ...
    if (value instanceof RegExp) {
      ...
      if (valueType === "function") {
        ...
        if (functionString.startsWith(classPrefix) && functionString.endsWith("}")) {
          const className = functionString
            .slice(classPrefix.length, functionString.indexOf("{"))
            .trim();
      ...
      const entries = Reflect.ownKeys(value)
        .map((key) => {
          ...
          try {
            ...
            }${key.toString()}: ${inspect(propertyValue, indentLevel + indentation, cache).trim()}`;
          } catch (error) {
